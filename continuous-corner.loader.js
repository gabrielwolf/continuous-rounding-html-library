 ...
 *
 * You can change the default scaling factor in CSS like
 * html {
 *   --continuous-rounding-scale: 0.7;
 * }
 *
 */
...
(function () {
    ...
    function applyRadius(el) {
        ...
        const scale = parseFloat(cs.getPropertyValue('--continuous-rounding-scale')) || DEFAULT_SCALE;
        ...
        for (let i = 0; i < finals.length; i++) {
            const m = finals[i].match(/^(-?\d*\.?\d+)([a-zA-Z%]*)$/);
            if (m && m[2] !== '%') {
